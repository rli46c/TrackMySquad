...
export const addProject = (projectData) => async (dispatch) => {
    ...
    try {        
        const res = await axios.post('/api/project/addProject', projectData, config);
...
export const updateProject = (projectData) => async (dispatch) => {
    ...
    const config = { headers: { 'Content-Type': 'application/json' } };
    try {
        const res = await axios.put(`/api/project/updateProjectDetails/${projectData._id}`, projectData, config);
        dispatch({
            type: UPDATE_PROJECT,
            payload: res.data
        });
