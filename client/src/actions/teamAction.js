...
export const addMember = (memberData) => async (dispatch) => {
    ...
    try {        
        const res = await axios.post('/api/team/addMemberProfile', memberData, config);
...
export const updateMember = (memberData) => async (dispatch) => {
    ...
    const config = { headers: { 'Content-Type': 'application/json' } };
    try {
        const res = await axios.put(`/api/team/updateMemberProfile/${memberData._id}`, memberData, config);
        dispatch({
