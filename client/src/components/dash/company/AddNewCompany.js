...
import { setAddCompanyDialog, getAllCompanyTypes, addCompany } from '../../../actions/companyAction';
...
export const AddNewCompany = ({ 
    ...
    return (
    <Dialog keepMounted disableBackdropClick onClose={ ()=>setAddCompanyDialog(false) }
    open={addCompanyDialogOpen} TransitionComponent={Transition} fullWidth={fullWidth} 
    maxWidth={maxWidth} aria-labelledby="add-company-profile" aria-describedby="add-company-modal">
        <DialogTitle className={ classes.dialogTitle } id="add-company-profile">
            ...
            <Close className={ classes.times } onClick={ ()=>setAddCompanyDialog(false) } />
        </DialogTitle>
        <DialogContent>
            <form className={classes.root} noValidate autoComplete="off" onSubmit={onSubmit}
                ...
                <FormControl variant="standard" className={classes.formControl} fullWidth>
                    ...
                    <NativeSelect value={ companyType } onChange={ (e)=>setCompanyType(e.target.value) } id="company-type" >
                        <option value="" />
                ...
                <TextField value={ name } onChange={ (e)=>setName(e.target.value) } label="Name" variant="outlined" fullWidth tabIndex="1" />
...
const mapDispatchToProps = {
    setAddCompanyDialog, getAllCompanyTypes, addCompany
};
