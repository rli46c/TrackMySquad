...
router.get('/', auth, async (req, res) => {
    try {
        ...
        const companiesList = await Companies.find({}).populate('companyType', 'companyType');
...
router.post('/addCompanyProfile', auth, async (req, res) => {
    ...
    try {
        ...
        const comTyp = await CompanyTypes.findOne({ _id: addedCompany.companyType }).select('companyType');
...
router.put('/updateCompanyProfile/:id', auth, async (req, res) => {
    ...
    try {
        const updatedCompany = await Companies.findByIdAndUpdate(req.params.id, req.body);
